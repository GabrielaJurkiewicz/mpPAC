...
function [Maps, MeanSignal, MessMaps, MeanFPsignal] = calcMeanMap(HighFreqSignal, maxes, lowfreq, Fs, cut, w, fAstart, fAend, fAstep, fA, fP_b, Nboot, margines)
    ...
    for trial = 1:size(HighFreqSignal,3)
        
        %% ---------------- ADJUST TO TRIAL ---------------------  
        signal   = squeeze(HighFreqSignal(1,:,trial));
        fPsignal = lowfreq{1,trial};
        mxs      = maxes{1,trial};
        mx       = mxs((mxs>(floor((cut+margines)*Fs)))&(mxs<(floor(size(signal,2))-floor((cut+margines)*Fs))));
        ...
        while ~isempty(mx)
            ...
            idx = mx(1,1);
            num = num + 1;
            mx(mx<(idx+floor(2*cut*Fs))) = []; %removing all maxima that locates inside time window 
